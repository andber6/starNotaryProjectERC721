...
it('can add the star name and star symbol properly', async() => {
    ...
    let user = accounts[1];
    let instance = await StarNotary.deployed();
    await instance.createStar('A new star!', starId, {from: user});

    let createdStar = await instance.tokenIdToStarInfo.call(starId);
    assert.equal(createdStar, 'A new star!');
    let contractName = await instance.name();
    assert.equal(contractName, 'BerganStar');
    let contractSymbol = await instance.symbol();
    assert.equal(contractSymbol, 'BGN');
});
